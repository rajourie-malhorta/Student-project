<?php
   ...
   if(isset($_POST['signup_btn']) && isset($_FILES["file"]["name"])) {
            ...
            if($rowCount > 0) {
                ...
                if((preg_match("/^[a-zA-Z ]*$/", $_first_name)) && (preg_match("/^[a-zA-Z ]*$/", $_last_name)) &&
                 ...
                 (preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password))){
                    ...
                    $sql = "INSERT INTO users (firstname, lastname, email, pwdUsers, b_image) VALUES ('$firstname', '$lastname', '$email', '$password_hash', 
                        '$name')";
